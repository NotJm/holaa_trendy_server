...
export class ProductService extends BaseService<Products> {
   ...
   */
  private async findProductByCode(code: string): Promise<Products> {
   ...
   */
  private async findProductByCategory(code: string): Promise<Products> {
    return await this.findOne({
      relations: ['category'],
      where: {
        category: {
   ...
   */
  private async findProductsByCategory(code: string): Promise<Products[]> {
   ...
   */
  private async findProductsByKeyword(keyword: string): Promise<Products[]> {
    return await this.find({
   ...
   */
  protected async deleteById(code: string): Promise<Products> {
    const entity = await this.findProductByCode(code);
  ...
  public async createOne(
    ...
    const { code, category, sizes, colors, subCategories } = createProductDto;
    ...
    const existsCategory =
      await this.categoriesService.findCategoryByCode(category);

    if (!existsCategory) {
      throw new NotFoundException(
        `La categoría con el código ${category} no existe`,
      );
    }

    const existsSubCategories =
      await this.subCategoriesService.findSubCategoriesByCodes(subCategories);
    ...
    const existsColors = await this.colorsService.findColorsByIDs(colors);
   ...
   */
  public async getProductsByCategory(category: string): Promise<Products[]> {
   ...
   */
  public async getProductsByKeyword(keyword: string): Promise<Products[]> {
  ...
  public async updateOne(
    ...
    if (updateProductDto.colors) {
      existsColors = await this.colorsService.findColorsByIDs(
   ...
   * @returns Respuesta
   */
  async deleteOne(code: string): Promise<Products> {
   ...
   */
  async deleteMany(codes: string[]): Promise<Products[]> {
